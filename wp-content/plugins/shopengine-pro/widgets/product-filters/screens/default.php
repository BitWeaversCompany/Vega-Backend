		...
		<div class="shopengine-filter-group">
			...
			<div id="shopengine-filter-group-content" class="shopengine-filter-group-content-wrapper">
				...
				<div class="shopengine-product-filters-wrapper" data-filter-price="<?php echo esc_attr($settings['shopengine_filter_toggle_price'], 'shopengine-pro'); ?>" data-filter-rating="<?php echo esc_attr($settings['shopengine_filter_toggle_rating'], 'shopengine-pro'); ?>" data-filter-color="<?php echo esc_attr($settings['shopengine_filter_toggle_color'], 'shopengine-pro'); ?>" data-filter-category="<?php echo esc_attr($settings['shopengine_filter_toggle_category'], 'shopengine-pro'); ?>" data-filter-attribute="<?php echo esc_attr($settings['shopengine_enable_attribute'], 'shopengine-pro'); ?>" data-filter-label="<?php echo esc_attr($settings['shopengine_enable_label'], 'shopengine-pro'); ?>" data-filter-image="<?php echo esc_attr($settings['shopengine_enable_image'], 'shopengine-pro'); ?>" data-filter-shipping="<?php echo esc_attr($settings['shopengine_enable_shipping'], 'shopengine-pro'); ?>" data-filter-stock="<?php echo esc_attr($settings['shopengine_enable_stock'], 'shopengine-pro'); ?>" data-filter-onsale="<?php echo esc_attr($settings['shopengine_enable_onsale'], 'shopengine-pro'); ?>" data-filter-brand="<?php echo esc_attr($settings['shopengine_filter_toggle_brand'], 'shopengine-pro'); ?>" data-filter-view-mode="<?php echo esc_attr($settings['shopengine_filter_view_mode'], 'shopengine-pro'); ?>">
					...
					if (isset($settings['shopengine_filter_toggle_brand']) && $settings['shopengine_filter_toggle_brand'] === 'yes') {

						// Check if the product_brand taxonomy exists (WooCommerce Brands functionality)
						if (taxonomy_exists('product_brand')) {
							
							$orderby = isset($settings['shopengine_filter_brand_orderby']) ? $settings['shopengine_filter_brand_orderby'] : 'name';
							$hierarchical = isset($settings['shopengine_filter_brand_hierarchical']) ? $settings['shopengine_filter_brand_hierarchical'] : true;
							$show_parent_only = isset($settings['shopengine_filter_brand_show_parent_only']) ? $settings['shopengine_filter_brand_show_parent_only'] : '';
							$hide_empty = isset($settings['shopengine_filter_brand_hide_empty']) ? $settings['shopengine_filter_brand_hide_empty'] : false;
							$except_brands = isset($settings['shopengine_filter_except_brand']) ? $settings['shopengine_filter_except_brand'] : [];
							...
							if ('order' === $orderby) {
								// Try different possible meta keys for brand ordering
								$args['orderby'] = 'term_order';
								$args['order'] = 'ASC';
								$test_brands = get_terms('product_brand', array_merge($args, ['number' => 1]));
							...
							// Exclude brands if specified
							if (!empty($except_brands)) {
								$args['exclude'] = $except_brands;
							}

							$product_brands = get_terms('product_brand', $args);

							$tplBrand = Products::instance()->get_widget_template($this->get_name(), 'brand', \ShopEngine_Pro::widget_dir());
