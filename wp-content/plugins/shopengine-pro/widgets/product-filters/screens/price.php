		...
		<div class="shopengine-filter-price-btns">
				<p class="shopengine-filter-price-result" data-sign="<?php echo esc_attr(get_woocommerce_currency_symbol()); ?>">
					<?php
						...
						echo sprintf($format, // min range format
							wp_kses(get_woocommerce_currency_symbol(), \ShopEngine_Pro\Util\Helper::get_kses_array()),// add min range currency symbol
							esc_html($default_range[0]))  .' - '. sprintf($format, wp_kses(get_woocommerce_currency_symbol(), \ShopEngine_Pro\Util\Helper::get_kses_array()), //add min range price and concat max range format with currency symbol
							esc_html($default_range[1]) // add max range price
						);
