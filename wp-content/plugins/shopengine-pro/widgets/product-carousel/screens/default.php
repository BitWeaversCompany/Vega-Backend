...
$carousel_config = [
	...
	'speed'		=> !empty($carousel_speed) ? $carousel_speed : 1000,
	'loop'		=> ($carousel_loop === 'yes') ? true : false,
	'arrow'		=> true,
	'dots'		=> ($carousel_dot === 'yes') ? true : false,
	'slidesPerView'	=> $slide_to_show['size'] ?? 4,
	'spaceBetween'	=> $slider_spacing['size'] ?? 30,
	'breakpoints'		=> [
		320 => [
			'slidesPerView'	=> 1,
		],
		360 => [
			'slidesPerView'	=> $slide_to_show_mobile['size'] ?? 1,
		],
		767 => [
			'slidesPerView'	=> $slide_to_show_tablet['size'] ?? 3,
		],
		1024 => [
			'slidesPerView'	=> $slide_to_show['size'] ?? 4,
		]
...
<div class="shopengine-product-carousel" data-carousel-config="<?php echo esc_attr(json_encode($carousel_config)); ?>">
	...
	<?php if ($productQuery->have_posts()): ?>
				...
				<?php while ($productQuery->have_posts()): $productQuery->the_post();
					...
					<div class="swiper-slide shopengine-single-product-item">
						...
						<div class="product-thumb">
							...
							<!-- product thumb -->
							<a title="<?php esc_attr_e('View Product Full Details','shopengine-pro')?>" href="<?php echo esc_url(get_the_permalink()); ?>">
								...
								<ul>
									...
									if($show_tag === "yes"){
										if($product_tags && !is_wp_error($product_tags)): 
											...
											<li class="badge tag">
												<a title="<?php echo esc_html($product_tag->name, 'shopengine-pro');?>" href="<?php echo esc_url(get_term_link($product_tag->term_id)); ?>">
												<?php esc_html_e($product_tag->name, 'shopengine-pro'); ?></a>
											</li>
									<?php endif;} ?> 
									<!-- on sale -->
									<?php
									if($show_sale === "yes"){ 
										...
										<li class='badge no-link sale'><?php esc_html_e('Sale!', 'shopengine-pro')?></li>
									<?php endif;} ?> 
									<!-- out of stock -->
									<?php
									if($show_stock_out_badge === "yes"){ 
										if(!$product->is_in_stock()):?>
											...
											<?php echo esc_html__('Out of Stock', 'shopengine-pro'); ?>
										</li>
									<?php endif;} ?> 
								</ul>
							</div>
									...
									<div class='product-category'>
										<ul>
											<?php foreach ($slice_product_cats as $key => $product_cat): ?>
												<li>
													<a title="<?php esc_html_e('Product Category','shopengine-pro')?>" href="<?php echo esc_url(get_term_link($product_cat)); ?>">
														<?php
						...
						<h3 class='product-title'>
							<a title="<?php esc_html_e('View Product Full Details','shopengine-pro')?>" href="<?php the_permalink();?>">
								<?php
		...
		<?php if($carousel_dot == 'yes') : ?>
			<div class="swiper-pagination"></div>
		<?php endif; ?>

		<?php if($carousel_arrow == 'yes') : ?>
			<div class="elementor-swiper-button swiper-button-prev">
				<?php if($arrow_type == 'icon') :
					\Elementor\Icons_Manager::render_icon( $left_arrow_icon, [ 'aria-hidden' => 'true' ]); 
				elseif($arrow_type == 'text') : ?>
					<span><?php echo esc_html($arrow_left_text); ?></span>
				<?php else :
					\Elementor\Icons_Manager::render_icon( $left_arrow_icon, [ 'aria-hidden' => 'true' ]); ?>
					<span><?php echo esc_html($arrow_left_text); ?></span>
				<?php endif; ?>
			...
			<div class="elementor-swiper-button swiper-button-next">
				<?php if($arrow_type == 'icon') :
					\Elementor\Icons_Manager::render_icon( $right_arrow_icon, [ 'aria-hidden' => 'true' ]);
				elseif($arrow_type == 'text') : ?>
					<span><?php echo esc_html($arrow_right_text); ?></span>
				<?php else : ?>
					<span><?php echo esc_html($arrow_right_text); ?></span>
					<?php \Elementor\Icons_Manager::render_icon( $right_arrow_icon, [ 'aria-hidden' => 'true' ]); 
				endif; ?>
			</div>
		<?php endif; ?>
	...
	<?php else:
			...
			esc_html_e('No product found.', 'shopengine-pro');
