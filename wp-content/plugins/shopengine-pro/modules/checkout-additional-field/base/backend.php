...
class Backend {
    ...
    private function order_details($order, $type) {
        ...
        if (isset($_GET['post_type']) && $_GET['post_type'] === 'shop_order') {
            ...
            // Fix the meta key for additional fields (empty type should use double underscore)
            $meta_key = '_shopengine_additional_' . $type . '_fields';
            if (empty($type)) {
                $meta_key = '_shopengine_additional__fields';
            }
            $fields = get_post_meta($order_id, $meta_key, true);
       ...
       <?php elseif ($type === 'shipping'): ?>
        ...
        <?php else: ?>
		    <h4><?php esc_attr_e('Additional order fields created from the shopengine', 'shopengine-pro')?><a title="<?php esc_html_e('Edit Additional Fields', 'shopengine-pro')?>" href="#" class="edit_address"></a></h4>
        <?php endif;?>
		<div class="address">
	    <?php
            foreach ($fields as $index => $value) {
                ...
                // Generate unique field name the same way as frontend
                $unique_name = !empty($value['name']) ? $value['name'] . '_' . (!empty($value['_uid']) ? $value['_uid'] : $index) : 'field_' . ($value['_uid'] ?? $index);
                $name = Checkout_Additional_Field::make_useable_input_name($unique_name);
                $field_value = get_post_meta($order_id, $name, true);
                // If field value is empty, try without the UID for backward compatibility
                if (empty($field_value)) {
                    $simple_name = Checkout_Additional_Field::make_useable_input_name($value['name']);
                    $field_value = get_post_meta($order_id, $simple_name, true);
                }
                echo wp_kses( "<p><strong>{$value['label']}:</strong>" . $field_value . "</p>", \ShopEngine\Utils\Helper::get_kses_array());
            }
        ?>
		</div>
